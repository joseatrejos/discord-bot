import asyncio, discord, json, os, Paginator, random, re, requests, time, wavelink, threading
...
@bot.command('register', aliases=["reg"], help='Registers the mentioned user (or yourself by default) to the database')
async def register(ctx, user: discord.Member = None):
    """Registers the mentioned user (or yourself by default) to the database"""

    if user is None:
        discord_user = ctx.author.id
        discord_name = ctx.author.name
        discord_nickname = ctx.author.nick
    else:
        discord_user = user.id
        discord_name = user.name
        discord_nickname = user.nick

    result = db.insert_discord_user(discord_user, discord_name, discord_nickname, PATH)
    if result is None:
        return await send_embed(ctx, 'User already registered.', f'{discord_name} is already registered in the database.', 0xFF0000)
    if type(result) is int:
        return await send_embed(ctx, 'User registered.', f'I have registered {discord_name} in the database.', 0x00FF00)
    else:
        return await send_embed(ctx, 'Error registering user.', f'An error has ocurred while trying to register {discord_name} in the database.', 0xFF0000)
...
# Birthday command to set a birthday for a user (Receives 0 arguments)
@bot.command('set_birthday', aliases=['setbday', 'setb','set_bday', 'set_b'], help='Set your birthday')
async def birthday(ctx):
    """Set your birthday"""

    try:
        birthday_received = ctx.message.content.split(' ', 1)[1]
    except:
        birthday_received = None

    birthday_regex = '^\d{1,2}/\d{1,2}/\d{4}$'
    birthday = None
    
    if (birthday_received != '' and birthday_received is not None):

        if (re.search(birthday_regex, birthday_received) is None):
            return await send_embed(ctx, "Invalid birthday", "Please use the following format: **dd/mm/yyyy**", 0xFF0000)
        else:
            birthday = format_date(birthday_received)
            
            # Cast to date in order to validate the date
            try:
                datetime.strptime(birthday, '%d/%m/%Y').date()
            except Exception as e:
                return await send_embed(ctx, "Invalid birthday", f"Please send a valid date. {e}", 0xFF0000)

            # Check if the user is already in our database and send the birthday to the database so it is saved if the user exists
            result = db.register_discord_user(ctx.author.id, ctx.author.name, ctx.author.nick, birthday, PATH)

    else:
        # Check if the user is already in our database
        result = db.register_discord_user(ctx.author.id, ctx.author.name, ctx.author.nick, None, PATH)


    # If the user was inserted because it didn't exist, we need to get the user's birthday
    if(result == "insert"):

        # If the user sent the birthday as an argument, we use that
        if(birthday != '' and birthday is not None):
            db.update_user_birthday(ctx.author.id, birthday, PATH)
            return await send_embed(ctx, "Birthday set", f"<@{ctx.author.id}> your birthday has been set to **{birthday}**", 0x00FF00, "https://hips.hearstapps.com/hmg-prod/images/birthday-cake-with-happy-birthday-banner-royalty-free-image-1656616811.jpg?crop=0.668xw:1.00xh;0.0255xw,0")
        
        else:
            # Ask the user for his birthday
            await send_embed(ctx, "Birthday", "Please enter your birthday in the following format: **dd/mm/yyyy**", 0xFF11FF)
            birthday = await wait_for_response(bot, ctx, 30) # Wait for a response for 30 seconds

            # Get the birthday from the message after checking the format and update it
            if(re.search(birthday_regex, birthday) is not None):
                ...
                try:
                    ...
                    return await send_embed(ctx, "Invalid birthday", f"Please send a valid date. {exception}", 0xFF0000)

                db.update_user_birthday(ctx.author.id, birthday, PATH)
                return await send_embed(ctx, "Birthday", f"Your birthday has been set to **{birthday}**", 0x00FF00, "https://hips.hearstapps.com/hmg-prod/images/birthday-cake-with-happy-birthday-banner-royalty-free-image-1656616811.jpg?crop=0.668xw:1.00xh;0.0255xw,0")
            
            else:
                return await send_embed(ctx, "Invalid birthday", "Please try again using the following format: **dd/mm/yyyy**", 0xFF0000)

    # If the user already existed in the DB
    elif(result == "update"):
        ...
        if(birthday is None):
            ...
            if(re.search(birthday_regex, birthday) is not None):
                ...
                db.update_user_birthday(ctx.author.id, birthday, PATH)
                return await send_embed(ctx, "Birthday", f"Your birthday has been updated to **{birthday}**", 0x00FF00, "https://hips.hearstapps.com/hmg-prod/images/birthday-cake-with-happy-birthday-banner-royalty-free-image-1656616811.jpg?crop=0.668xw:1.00xh;0.0255xw,0")
            
            else:
                return await send_embed(ctx, "Invalid birthday", "Please use the following format: **dd/mm/yyyy**", 0xFF0000)
        
        else:
            # Since the user already existed and the birthday was sent as an argument, we updated it upon calling register_discord_user()
            return await send_embed(ctx, "Birthday", f"Your birthday has updated set to **{birthday}**", 0x00FF00, "https://hips.hearstapps.com/hmg-prod/images/birthday-cake-with-happy-birthday-banner-royalty-free-image-1656616811.jpg?crop=0.668xw:1.00xh;0.0255xw,0")

    else:
        return await send_embed(ctx, "Error", "Something went wrong", 0xFF0000)

# Command to get the birthday of N users
@bot.command('birthday', aliases=['bday', 'b'], help='Get the birthday of 1 user (or more users with the today flag)')
async def birthday(ctx, birthday = None):
    """Get the birthday of 1 or more users"""

    today_flag = False
    try:
        today_flag = True
        argument = ctx.message.content.split(' ', 1)[1].lower().strip()

        # If the argument is today, we get the users whose birthday is today
        if(argument == 'today'):
            birthdays = db.get_today_birthdays(PATH)

            if len(birthdays) == 0:
                return await send_embed(ctx, "No birthdays today", "There are no birthdays today", 0x0000FF)
            ...
            for i, user in enumerate(birthdays):

                # Now construct the embed item with birthday information 
                description += f'*{user[1]}* enjoy your birthday!'
                string_length += len(f'*{user[1]}* enjoy your birthday!') + 2
                total_embed_length = len(description) + len(title) + 2

                if(string_length > 800 or i == len(birthdays) - 1):
                    # Insert our template embed into the embeds list and then modify it with the constructed values
                    embed = discord.Embed(
                        title=title,
                        description=f'{description}',
        ...
        elif("<@" in argument == False):
            return await send_embed(ctx, "Invalid argument", "Please use the today flag", 0xFF0000)
        ...
        if(today_flag == False):
            # If there are mentions, get the birthday of each user
            ...
            if(len(mentions) > 0):
                ...
                for i, mention in enumerate(mentions):
                    ...
                    birthday = db.get_user_birthday(mention.id, PATH)

                    # Now add the song information into the currently inserted embed
                    description += f'{i+1}. __{mention.name}__\'s birthday is on **{birthday}**\n\n'
                    string_length += len(f'{i+1}. __{mention.name}__\'s birthday is on **{birthday}**') + 2
                    total_embed_length = len(description) + len(title) + 2

                    if(string_length > 800 or i == len(mentions) - 1):
                        # Insert our template embed into the embeds list and then modify it with the constructed values
                        embed = discord.Embed(
                            title=title,
                            description=f'{description}',
            ...
            # If there are no mentions, just get the author's birthday
            else:
                birthday = db.get_user_birthday(ctx.author.id, PATH)
                if(birthday is not None):
                    return await send_embed(ctx, "Birthday", f"<@{ctx.author.id}> your birthday is **{birthday}**", 0x0000FF, "https://hips.hearstapps.com/hmg-prod/images/birthday-cake-with-happy-birthday-banner-royalty-free-image-1656616811.jpg?crop=0.668xw:1.00xh;0.0255xw,0")
                else:
                    return await send_embed(ctx, "Birthday not yet set", f"<@{ctx.author.id}> you haven't set your birthday yet. Use -set_birthday first.", 0xFF0000)

# Command to get all the birthdays of the server
@bot.command('get_all_birthdays', aliases=["get_birthdays", "birthdays", "bdays"], help='Get all the birthdays')
async def get_birthdays(ctx):
    """Get all the birthdays"""

    birthdays = db.get_all_user_birthdays(PATH)
    
    if len(birthdays) == 0:
        return await send_embed(ctx, "No birthdays", "There are no birthdays", 0x0000FF)
    
    return await send_birthday_pagination(ctx, birthdays)

# Command to set a timer for a certain amount of seconds
@bot.command('timer', help='Set a timer for a certain amount of time. You\'ll be notified when the timer is done.')
async def timer(ctx, seconds):
    """Start a timer"""
    timer_status = db.get_timer_status(PATH)

    if timer_status == 0:

        db.toggle_timer(PATH)
        try:
            secondint = int(seconds)
            if secondint <= 0:
                await ctx.send("My creator didn\'t allow me to do negatives.")
                raise BaseException

            if secondint > 7200:
                return await ctx.send("My creator didn\'t allow me to do more than 2 hours per use.")
            ...
            await send_embed(ctx, "Invalid argument", "Please send a valid **number** as an argument.", 0xFF0000)
    else:
        await send_embed(ctx, "Timer already running", "There is already a timer running. Please wait for it to finish before starting a new one.", 0xFF0000)
...
# Command to "disconnect" the bot's voice channel and then reconnect it
@bot.command('soft_reset', aliases=["reset"], help='Restart the bot in case music gets buggy')
async def soft_reset(ctx):
    """Stop all music and reset the bot"""
    await ctx.bot_vc.stop()
    await ctx.bot_vc.disconnect()
    custom_player = CustomPlayer()
    bot_vc: CustomPlayer = await ctx.author.voice.channel.connect(cls=custom_player)
    return await send_embed(ctx, "Reset", "The bot has been reset", 0x0000FF)

# Command to save a meeting in the database
@bot.command('meeting', aliases=["meet", "daily", "mt"], help='Start a meeting')
async def meeting(ctx):
    """Save a meeting"""

    try:
        # Get the date and format it
        meeting_date = ctx.message.content.split(' ')[1].strip()
        meeting_date = format_date(meeting_date)
        datetime.strptime(meeting_date, '%d/%m/%Y').date()
        
        # Construct the attendees' list
        mentions = ctx.message.mentions
        attendees = []
        for mention in mentions:
            author_dict = construct_author_dict(mention, "mention")
            attendee_db_id = db.get_or_createuser_by_discord_id(author_dict, PATH)
            attendees.append(attendee_db_id)
        author_dict = construct_author_dict(ctx)
        attendee_db_id = db.get_or_createuser_by_discord_id(author_dict, PATH)
        attendees.append(attendee_db_id)
        ...
        for role_mention in role_mentions:
            roles.append(f"&{role_mention.id}")

        # Ask for the time of the meeting
        if len(attendees) == 1:
            await send_embed(ctx, "Time of the meeting", "At what time is the meeting? **(HH:MM)**.\n\n**Note:** you seem to be the only participant. To add more attendees, send the meeting date and mention your @companions", 0xFF11FF)
        else:
            await send_embed(ctx, "Time of the meeting", "At what time is the meeting? **(HH:MM)**", 0xFF11FF)
        
        # Await for an answer with the time and then format said response 
        try:
            meeting_time = await wait_for_response(bot, ctx, 30)  # Wait for a response for 30 seconds
            meeting_time = meeting_time.strip()

            try:
                meeting_time = format_time(meeting_time)
            except IndexError:
                return await send_embed(ctx, "Invalid time", "Please send a valid time in the format **HH:MM**", 0xFF0000)

        except asyncio.TimeoutError:
            return await send_embed(ctx, "Timeout", "You didn't respond in time.", 0xFF0000)

        if(meeting_time is None):
            return await send_embed(ctx, "No response", "You didn't respond in time. Please try again.", 0xFF0000)

        
        # Now validate the time
        if validate_time(meeting_time) == False:
            return await send_embed(ctx, "Invalid time", "The time you entered is invalid. Please try again.", 0xFF0000)
        
        # Finally, ask for the meeting's description
        await send_embed(ctx, "Meeting name", "What's the name of your meeting?", 0xFF11FF)
        try:
            meeting_name = await wait_for_response(bot, ctx, 30)  # Wait for a response for 30 seconds
            meeting_name = meeting_name.strip()
        except asyncio.TimeoutError:
            return await send_embed(ctx, "Timeout", "You didn't respond in time.", 0xFF0000)

        # Save the meeting in the database
        db.save_meeting(meeting_date, meeting_time, meeting_name, attendees, roles, PATH)
        return await send_embed(ctx, "Meeting saved", f"The meeting **{meeting_name}** has been saved for **{meeting_date}** at **{meeting_time}**", 0x00FF00)

    except IndexError:
        traceback.print_exc()
        exception = traceback.format_exc()
        return await send_embed(ctx, "No argument sent", "Please send a valid **date** *(dd/mm/yyyy)* as an argument.", 0xFF0000)
    except TypeError:
        traceback.print_exc()
        exception = traceback.format_exc()
        return await send_embed(ctx, "Invalid argument", "Please send a valid **date** *(dd/mm/yyyy)* as an argument.", 0xFF0000)
    except Exception:
        traceback.print_exc()
        exception = traceback.format_exc()
        return await send_embed(ctx, "Unexpected error", f"An unexpected error occured: {exception}. Please try again later.", 0xFF0000)

# Command to get all your meetings
@bot.command('get_meetings', aliases=["get_meeting", "getm", "get_m", "meetings", "meets", "ms", "mts"], help='Get all your meetings')
async def get_meetings(ctx):
    """Get all your meetings"""
    
    meetings = db.get_user_meetings(ctx.author.id, PATH)
    
    if len(meetings) == 0:
        return await send_embed(ctx, "No meetings", "There are no meetings", 0x0000FF)
    
    return await send_meetings_pagination(ctx, meetings)

# Command to delete all meetings
@bot.command('delete_meetings', aliases=["delete_meeting", "delm", "del_m", "del_meets", "delmeets", "delms", "delmts"], help='Delete all your meetings')
async def delete_meetings(ctx):
    """Delete all your meetings"""
    
    if check_admin_authors(ctx.author.id) == False:
        return await send_embed(ctx, "Unauthorized", "You are not authorized to use this command", 0xFF0000)

    deleted_meetings = db.delete_all_meetings(PATH)

    if deleted_meetings == "":
        return await send_embed(ctx, "No meetings to delete", "There are no meetings in the DB", 0x0000FF)

    return await send_embed(ctx, "All meetings have been deleted", f"{deleted_meetings}", 0x00FF00)

# Command to save a note in the database
@bot.command('save_note', aliases=["saven", "save_n", "sn", "note", "n"], help='Save a note')
async def notes(ctx):
    """Save a note"""

    try:
        note = ctx.message.content.split(' ', 1)[1]

        try:
            subject = note.split('|', 1)[0].strip()
            ...
            author_id = ctx.author.id
            author_name = ctx.author.name
            author_nickname = ctx.author.display_name
            db.save_note(author_id, author_name, author_nickname, subject, note, PATH)                
            return await send_embed(ctx, "Note saved", f"The note has been saved", 0x00FF00)

    except IndexError:
        return await send_embed(ctx, "No argument sent", "Please send a valid **note** as an argument.", 0xFF0000)

# Command to get all your notes
@bot.command('get_notes', aliases=["getn", "get_n", "gn", "notes"], help='Get all your notes')
async def get_notes(ctx):
    """Get all your notes"""

    author_dict = construct_author_dict(ctx)
    user_id = db.get_or_createuser_by_discord_id(author_dict, PATH)
    notes = db.get_user_notes(user_id, PATH)

    if notes is None:
        return await send_embed(ctx, "No notes", "There are no notes", 0x0000FF)
    
    if len(notes) == 0:
        return await send_embed(ctx, "No notes", "There are no notes", 0x0000FF)
    
    return await send_note_pagination(ctx, notes)

# Command to get every single note that's related to a subject
@bot.command('get_subject', aliases=["get_subjects", "get_s", "gs", "subject"], help='Get all your notes in a subject')
async def get_subject(ctx):
    """Get all the notes in a subject"""

    try:
        subject = ctx.message.content.split(' ', 1)[1].strip()

        notes = db.get_notes_by_subject(subject, PATH)

        if notes is None:
            return await send_embed(ctx, "Subject does not exist", f"The subject **{subject}** doesn't exist.", 0x0000FF)

        if len(notes) == 0:
            return await send_embed(ctx, "No notes", "There are no notes in said subject.", 0x0000FF)
        
        return await send_subject_pagination(ctx, subject, notes)

    except IndexError:
        return await send_embed(ctx, "No argument sent", "Please send a valid **subject** as an argument.", 0xFF0000)

# Command to delete a note
@bot.command('delete_note', aliases=["deln", "deleten", "delete_n", "del_note", "del_n", "dn"], help='Delete a note')
async def delete_notes(ctx):
    """Delete a note"""

    try:
        note_id = ctx.message.content.split(' ', 1)[1]
        author_id = ctx.author.id
        result = db.delete_note_by_id(note_id, author_id, PATH)

        if result == "inexistent_user":
            return await send_embed(ctx, "Inexistent user", "You're not yet registered in Ludens's DB. Please use the -note command or -setb command to register yourself", 0xFF0000)

        if result == "inexistent_note":
            return await send_embed(ctx, "Inexistent note", "The note you're trying to delete doesn't exist.", 0xFF0000)
        
        return await send_embed(ctx, "Note deleted", f"The note has been deleted", 0x00FF00)

    except IndexError:
        return await send_embed(ctx, "No argument sent", "Please send a valid **note id** as an argument.", 0xFF0000)

# Command to delete all of your notes
@bot.command('delete_notes', aliases=["delns", "deletens", "delete_ns", "del_notes", "del_ns", "dns"], help='Delete a note')
async def delete_notes(ctx):
    """Delete all of your notes"""

    author_id = ctx.author.id
    db.delete_all_user_notes(author_id, PATH)
    return await send_embed(ctx, "Notes deleted", f"All of your notes have been deleted", 0x00FF00)

# Command to create a reminder in the database (in minutes)
@bot.command('reminder', aliases=["rem", "remind"], help='Create a reminder')
async def remind(ctx):
    """Receive the amount of minutes in which you want to be reminded of a certain topic"""
    
    try:
        reminder_time = ctx.message.content.split(' ')[1].strip()
        ...
        if(reminder_time.isdigit()):
            ...
            try:
                ...
                if(reminder_time < 1):
                    return await send_embed(ctx, "Invalid time", "Please send a valid positive amount of **minutes** as an argument.", 0xFF0000)
            
            except ValueError:
                return await send_embed(ctx, "Invalid time", "Please send a valid positive amount of **minutes** as an argument.", 0xFF0000)
            
            # Now check if the user already sent a reminder topic
            try:
                reminder_topic = ctx.message.content.split(' ', 2)[2].strip()
            except:
                # if no reminder was sent, ask for it
                await send_embed(ctx, "Reminder topic", "What do you wish to be reminded about?", 0x0000FF)
                try:
                    reminder_topic = await wait_for_response(bot, ctx, 30)  # Wait for a response for 30 seconds
                    reminder_topic = reminder_topic.strip()
                except asyncio.TimeoutError:
                    return await send_embed(ctx, "Timeout", "You didn't respond in time.", 0xFF0000)
            ...
            try:
                db.save_reminder(author_dict, int(reminder_time), reminder_topic, PATH)
                return await send_embed(ctx, "Reminder saved", f"In **{reminder_time}** minutes, you will be reminded of: {reminder_topic}", 0x00FF00)
            except Exception:
                traceback.print_exc()
                error = traceback.format_exc()
                return await send_embed(ctx, "Error", f"An error has occurred: {error}", 0xFF0000)
        else:
            return await send_embed(ctx, "Invalid time", "Please send a valid amount of **minutes** as an argument.", 0xFF0000)

    except IndexError:
        return await send_embed(ctx, "No argument sent", "Please send a valid amount of **minutes** as an argument.", 0xFF0000)

# Command to get all your reminders
@bot.command('get_reminders', aliases=["getr", "get_r", "get_rems", "gr", "reminders"], help='Get all your reminders')
async def get_reminders(ctx):
    """Get all your reminders"""

    author_dict = construct_author_dict(ctx)
    user_id = db.get_or_createuser_by_discord_id(author_dict, PATH)
    reminders = db.get_user_reminders(user_id, PATH)

    if reminders is None:
        return await send_embed(ctx, "No reminders", "There are no reminders", 0x0000FF)
    
    if len(reminders) == 0:
        return await send_embed(ctx, "No reminders", "There are no reminders", 0x0000FF)
    
    return await send_reminders_pagination(ctx, reminders)

# Command to create a recurring meeting
@bot.command('recurring_meeting', aliases=["r_meet", "rec_m", "recm", "meetr", "meetrec", "meet_rec", "meet_r", "rmt", "rmeeting", "rec_meeting", "recmeeting"], help='Create a recurring meeting')
async def recurring_meeting(ctx):
    """Create a recurring meeting"""
    
    try:
        # Get the meeting days
        meeting_days = ctx.message.content.split(' ', 1)[1].strip().split('|')[0].strip()
    except IndexError:
        return await send_embed(ctx, "No argument sent", "Please send valid **weekdays** (separated by commas) as an argument.", 0xFF0000)

    meeting_days = meeting_days.split(',')
    if len(meeting_days) > 7:
        return await send_embed(ctx, "Too many arguments sent", "Please send a valid number of **weekdays** (separated by commas) as an argument.", 0xFF0000)

    meeting_days = validate_weekdays(meeting_days)

    if meeting_days == False:
        return await send_embed(ctx, "Repeated weekdays", "You must not repeat **weekdays** in the argument.", 0xFF0000)
    if meeting_days[0] == "invalid_weekday":
        return await send_embed(ctx, f"Invalid day detected {meeting_days[1]}", "Please send valid **weekdays** (separated by commas) as an argument.", 0xFF0000)
    
    # Check if the user already sent a meeting time
    try:
        meeting_time = ctx.message.content.split(' ', 1)[1].strip().split('|')[1].strip()
    except:
        # If no meeting time was sent, ask for it
        await send_embed(ctx, "Meeting time", "At what time is the meeting?", 0x0000FF)
        try:
            meeting_time = await wait_for_response(bot, ctx, 30)  # Wait for a response for 30 seconds
            meeting_time = meeting_time.strip()
        except asyncio.TimeoutError:
    ...
    # Now check if the user already sent a meeting topic
    try:
        meeting_topic = ctx.message.content.split(' ', 1)[1].strip().split('|')[2].strip()
    except IndexError:
        # If no meeting topic was sent, ask for it
        await send_embed(ctx, "Meeting topic", "What is the topic of the meeting?", 0x0000FF)
        try:
            meeting_topic = await wait_for_response(bot, ctx, 30)  # Wait for a response for 30 seconds
            meeting_topic = meeting_topic.strip()
        except asyncio.TimeoutError:
            return await send_embed(ctx, "Timeout", "You didn't respond in time.", 0xFF0000)

    # Finally, check if the user mentioned the attendees
    discord_mentions = ctx.message.mentions
    if len(discord_mentions) == 0:
        # If no meeting attendees were mentioned, ask for them
        await send_embed(ctx, "Meeting attendees", "Who are the attendees of the meeting?", 0x0000FF)
        try:
            # Wait for a response from the original author (for 30 seconds)
            meeting_attendees = await bot.wait_for('message', timeout=30, check=lambda message: message.author == ctx.author)
            meeting_attendees = meeting_attendees.content.strip()
            meeting_attendees = get_mention_ids(meeting_attendees)

        except asyncio.TimeoutError:
            return await send_embed(ctx, "Timeout", "You didn't respond in time.", 0xFF0000)
    else:
        # Validate that the mentioned users are the last argument in the command
        if '<@' in ctx.message.content and '>' in ctx.message.content:
            try:
                validate_mentions = ctx.message.content.split(' ', 1)[1].strip().split('|')[3].strip()
            except:
                return await send_embed(ctx, "Invalid mentions", "Please mention the attendees of the meeting as the last parameter of the arguments (separated by pipes).", 0xFF0000)

        meeting_attendees = []
        for mention in discord_mentions:
            meeting_attendees.append(mention.id)

    attendees = []
    for attendee in meeting_attendees:
        user_id = db.just_get_user_by_discord_id(attendee, PATH)
        if user_id is None:
            return await send_embed(ctx, "Invalid attendee", f"User <@{attendee}> is not registered in the database.", 0xFF0000)
    ...
    # Now save the recurring meetings in the database
    db.save_recurring_meeting(meeting_days, meeting_time, meeting_topic, attendees, roles, PATH)
    return await send_embed(ctx, "Recurring meeting saved", f"Your recurring meeting has been saved.", 0x00FF00)

# Command to get all recurring meetings
@bot.command('get_recurring_meetings', aliases=["get_rms", "get_rmts", "r_meets", "rec_ms", "recms", "meetsr", "meetrs", "meet_rs", "grmts", "grms", "recurring_meetings", "rec_meetings", "recmeetings"], help='Get all recurring meetings')
async def get_recurring_meetings(ctx):
    """Get all recurring meetings"""

    recurring_meetings = db.get_all_recurring_meetings(PATH)    # id, meeting_name, time
    
    if len(recurring_meetings) == 0:
        return await send_embed(ctx, "No recurring meetings", "There are no recurring meetings", 0x0000FF)
    
    return await send_recurring_meetings_pagination(ctx, recurring_meetings)

# Command to delete all recurring meetings
@bot.command('delete_recurring_meetings', aliases=["del_rms", "del_rmts", "del_r_meets", "del_rec_ms", "del_recms", "delmeetsr", "delmeetrs", "del_meet_rs", "drmts", "drms", "del_recurring_meetings", "del_rec_meetings", "delrecmeetings"], help='Delete all recurring meetings')
async def delete_recurring_meetings(ctx):
    """Delete all recurring meetings"""

    if check_admin_authors(ctx.author.id) == False:
        return await send_embed(ctx, "Unauthorized", "You are not authorized to use this command.", 0xFF0000)

    deleted_meetings = db.delete_recurring_meetings(PATH)
    if deleted_meetings == "":
        return await send_embed(ctx, "No recurring meetings to delete", "There are no recurring meetings in the DB", 0x0000FF)

    return await send_embed(ctx, "All recurring meetings deleted", f"{deleted_meetings}", 0x00FF00)

# Secondary event loop to check for reminders & meetings
async def event_check():
    """Check for any programmed events"""
    day_of_the_week = datetime.now(pytz.timezone('US/Arizona')).weekday()
    birthday = False
    code_review = False
    code_review_reminder = False

    # Definition of the events' discord webhook
    if os.getenv("DEBUG") == "True":
        url = os.getenv("TESTS_WEBHOOK_URL")
    else:
        url = os.getenv('MEETINGS_WEBHOOK_URL')
    payload = {}
    files={}
    headers = {"Content-Type": "application/json"}
    ...
    while True:
        ...
        current_minute = datetime.now(pytz.timezone('US/Arizona')).minute
        current_hour = datetime.now(pytz.timezone('US/Arizona')).hour
        current_day = datetime.now(pytz.timezone('US/Arizona')).weekday()

        # Once the day changes
        if day_of_the_week != current_day:
            ...
            # On debug mode, delete all recurring meetings
            if os.getenv("DEBUG_THREADS") == "True":
                deleted_recurring_meetings = db.delete_recurring_meetings(PATH)
                send_message_to_webhook(webhook_data, f"**Deleted all recurring_meetings** without exception", "Deleted meetings", deleted_recurring_meetings)

            # Delete all meetings that have already passed
            deleted_meetings = db.delete_meetings_before_today(PATH)
            if deleted_meetings != "":
                send_message_to_webhook(webhook_data, f"**Deleted all failed meetings** that were older than 1 day", "Deleted meetings", deleted_meetings)

            # Delete all reminders that weren't sent and hence are older than 1 day
            deleted_reminders = db.delete_reminders_before_today(PATH)
            if deleted_reminders != "":
                send_message_to_webhook(webhook_data, f"**Deleted all failed reminders** that were older than 1 day", "Deleted reminders", deleted_reminders)


        # Check if the current time is 11:00 AM to send the birthday message
        if(current_hour == 11 and birthday == False):
            birthday = True
            await birthday_check()

        # Code review shall start at 6:00 PM
        if(current_hour == 17 and current_minute == 40 and code_review == False):
            code_review = True
            await code_review_check()

        # Code review reminder will be sent 7 hours before the code review
        if(current_hour == 11 and code_review_reminder == False):
            code_review_reminder = True
            await code_review_reminder_check()

        # Check if there are events in the DB
        events = db.check_events(PATH)
        
        # Now send a request to our discord a webhook (when it applies) to send an alert
        for event in events:
            if event["event_type"] == "reminder":
                manage_reminder(event, url, payload, files, headers)

            elif event["event_type"] == "meeting":
                manage_meeting(event, url, payload, files, headers)

            elif event["event_type"] == "recurring_meeting":
                manage_recurring_meeting(event, url, payload, files, headers)

        if os.getenv("DEBUG_THREADS") == "True":
            await asyncio.sleep(TIME_CHECK)
        else:
            # Get the current time and sleep until the next minute
            current_time = datetime.now(pytz.timezone('US/Arizona'))
            next_minute = current_time + timedelta(minutes=1)
            next_minute = next_minute.replace(second=0, microsecond=0)
            sleep_time = (next_minute - datetime.now(pytz.timezone('US/Arizona'))).total_seconds()
            await asyncio.sleep(sleep_time)



async def code_review_check():
    """Check if it's code review day"""

    # Get the current date in Arizona timezone
    today = datetime.now(pytz.timezone('US/Arizona'))

    if today.weekday() == 0 or today.weekday() == 3:
        url = os.getenv('CODE_REVIEW_WEBHOOK_URL')
        payload = {}
        files={}
        headers = {"Content-Type": "application/json"}
        ...
        # Now send discord a webhook to the birthday channel
        payload={
            "content": f"__**Code Review About to Start!**__ <@&{os.getenv('LUDENS_GROUP_ID')}>",
            "embeds": [
                {
                    "title": "Code Review Session at 6:00 PM",
                    "description": f"Today is code review day! <@&{os.getenv('LUDENS_GROUP_ID')}>",
                    "color": 0x0000FF,
                    "thumbnail": {
                        "url": "https://blog.kiprosh.com/content/images/2020/10/what-is-code-review--2-.jpg"
                    }
                }
            ]
        }
...
async def code_review_reminder_check():
    """Remind everyone of the code review 6 hours before"""
    ...
    if today.weekday() == 0 or today.weekday() == 3:
        ...
        # Now send discord a webhook to the birthday channel
        payload={
            "content": f"__**Code Review Reminder!**__ <@&{os.getenv('LUDENS_GROUP_ID')}>",
            "embeds": [
                {
                    "title": "Code Review Session will be today at 6:00 PM",
                    "description": f"Today is code review day! <@&{os.getenv('LUDENS_GROUP_ID')}>",
                    "color": 0x0000FF,
                    "thumbnail": {
                        "url": "https://blog.kiprosh.com/content/images/2020/10/what-is-code-review--2-.jpg"
...
async def main():
    ...
    async with bot:
        # Start the event check loop in another thread
        thread = threading.Thread( target = asyncio.run, args = (event_check(),) )
        thread.start()
...
if os.getenv('DEBUG') == "True":    # For debugging purposes

    if os.getenv('DEBUG_THREADS') == "True":
        asyncio.run(event_check())
    else:
        bot.run(TOKEN)
